...
export const VoicePoweredOrb = ({
  ...
  // Initialize microphone access with external stream
  const initMicrophoneFromStream = async (stream) => {
    try {
      ...
      console.warn("Error initializing audio analysis:", error);
      return false;
    }
  };

  // Initialize microphone access (standalone mode)
  const initMicrophone = async () => {
    try {
      ...
      return initMicrophoneFromStream(stream);
    } catch (error) {
  ...
  // Handle external audio stream
  useEffect(() => {
    if (audioStreamRef?.current && enableVoiceControl) {
      initMicrophoneFromStream(audioStreamRef.current);
    }
  }, [audioStreamRef?.current, enableVoiceControl]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Initialize microphone if no external stream provided
      if (enableVoiceControl && !audioStreamRef?.current) {
        initMicrophone().then((success) => {
          isMicrophoneInitialized = success;
        });
      } else if (enableVoiceControl && audioStreamRef?.current) {
        initMicrophoneFromStream(audioStreamRef.current).then((success) => {
          isMicrophoneInitialized = success;
        });
      } else {
        stopMicrophone();
      ...
      const update = (t) => {
        ...
        if (enableVoiceControl && (isMicrophoneInitialized || analyserRef.current)) {
